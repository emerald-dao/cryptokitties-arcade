<script lang="ts">
	...
	$: activeCourse = path[2] + '/' + path[3];
	$: isCurrentChapter = chapter.slug === activeCourse;
</script>

<div class="{isCurrentChapter ? 'bg-black text-white' : ''} flex h-8 items-center">
	<a
		href="/courses/{chapter.slug}"
		class="font-vt323 w-fit px-3 text-base
     uppercase"
	>
		{number}. {chapter.name}
	</a>
